...
test('download boundary dedupe keeps one candle per timestamp',()=>{const a={timestamp:1000,open:1,high:2,low:.5,close:1.5};const b={...a,close:1.6};const out=dedupeCandles([a,b,{...a,timestamp:2000}]);assert.equal(out.length,2);assert.equal(out[0].close,1.6);});
