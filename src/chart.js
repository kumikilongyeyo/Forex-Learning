...
export function renderCandles(container, candles, { showEma = false, levels = [], height = 420 } = {}) {
  container.innerHTML = '';
  if (!candles.length) {
  ...
  const pad = { t: 20, r: 70, b: 30, l: 12 };
  const levelPrices = levels.map(l => Number(l.price)).filter(Number.isFinite);
  const max = Math.max(...candles.map(c => c.high), ...levelPrices);
  const min = Math.min(...candles.map(c => c.low), ...levelPrices);
  ...
  for (let i = 0; i <= 4; i += 1) {
    ...
    label.textContent = price.toFixed(price >= 20 ? 3 : 5);
    svg.append(label);
  }
  ...
  levels.forEach(level => {
    const yy = y(Number(level.price));
    const cls = level.kind ? `level-line ${level.kind}` : 'level-line';
    svg.append(el('line', { x1: pad.l, x2: width - pad.r, y1: yy, y2: yy, class: cls }));
    const label = el('text', { x: pad.l + 6, y: yy - 5, class: `level-label ${level.kind || ''}` });
    label.textContent = level.label || String(level.price);
    svg.append(label);
  });
